...
# Limites globales de calendario
...
def procesar_archivos(uploaded_files):
    ...
    for file in uploaded_files:
        try:
            ...
            alias_haz = ['horizontal_beamwidth', 'sector', 'haz', 'beam', 'sector_id']
            ...
            alias_ope = ['operador', 'proveedor', 'red', 'operator']
            ...
            col_ope = buscar_columna(df.columns, alias_ope)
            ...
            df_temp['operador'] = df[col_ope].astype(str).str.upper() if col_ope else 'DESCONOCIDO'
...
if st.session_state.df_master is not None:
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["📍 Búsqueda Geográfica", "⚙️ Inteligencia de Red", "📝 Informe PNL", "🕵️‍♂️ Búsqueda Victimarios", "👤 Patrón y Ficha Técnica"])
        ...
        if st.session_state.run_t1:
            ...
            if lista_nums:
                ...
                if not df_u.empty:
                    ...
                    # Generar Sinopsis basada en la evidencia real
                    sinop_txt = "<ul>"
                    for obj in lista_nums:
                        ...
                        if not df_obj_sinop.empty:
                            ...
                            sinop_txt += f"<li>El objetivo <b>{obj}</b> presenta su mayor zona de permanencia (residencia o trabajo) en la celda <b>{top_lugar}</b> con {n_vis} registros.</li>"
                        else:
                    ...
                    for rank_idx, (_, row) in enumerate(df_coords.iterrows()):
                        rank = rank_idx + 1
                        # FUCSIA si hay más de 1 objetivo, sino el color asignado
                    ...
                    # Trazar RUTA cronológica por objetivo
                    for obj in lista_nums:
    ...
    # ------------------------------------------
    # TAB 2: INTELIGENCIA DE RED (CON FILTROS AVANZADOS)
    # ------------------------------------------
    with tab2:
        ...
        if st.session_state.run_t2:
            with st.spinner("Procesando redes y topologías..."):
                ...
                if not df_ana.empty:
                    ...
                    # Encuentros
                    df_ana['vent'] = df_ana['fecha_limpia'].dt.floor(tol_enc.replace('min','T').replace('H','H'))
                    encuentros_list = []
                    
                    for (celda, vent), df_g in df_ana.groupby(['celda', 'vent']):
                        nums = df_g['numero_limpio'].unique()
                        if 1 < len(nums) <= 150:
                            ops = ", ".join(df_g['operador'].unique())
                            for pair in itertools.combinations(sorted(nums), 2):
                                encuentros_list.append({'Número A': pair[0], 'Número B': pair[1], 'Operador': ops})
                    ...
                    if encuentros_list:
                        ...
                        top_e_op = df_e.groupby(['Número A', 'Número B', 'Operador']).size().reset_index(name='Coincidencias').sort_values('Coincidencias', ascending=False).head(10)
                    else:
                        top_e_all = pd.DataFrame()
                        top_e_op = pd.DataFrame()

                    st.markdown('<div class="caja-info">💡 <b>Sinopsis de Red:</b> Análisis procesado exitosamente aplicando las tolerancias y filtros de tiempo especificados.</div>', unsafe_allow_html=True)
    ...
    # ------------------------------------------
    # TAB 3: INFORME PNL AVANZADO (CORRELACIÓN)
    # ------------------------------------------
    with tab3:
        st.header("📝 Informe PNL (Correlación de Relatos y Evidencia)")
        st.markdown("Extrae entidades (líneas) de documentos anexos y correlaciona Modo, Tiempo y Lugar exactos basados en la tabla. Incluye descarga KML de los involucrados.")
        ...
        if st.session_state.run_t3:
            ...
            if nums_implicados:
                ...
                if not df_c.empty:
                    # Sinopsis Criminal (Basada en los datos reales filtrados)
                    ...
                    <div class="caja-info">
                        ...
                        <p><b>Números Correlacionados:</b> {', '.join(df_c['numero_limpio'].unique())} ({n_activos} activos encontrados en el relato operando en este rango).</p>
                        ...
                        <p><b>📍 Lugar:</b> Convergencia principal de estos objetivos en las celdas: <b>{', '.join([str(c) for c in celdas_comunes if c != 'Desconocida'])}</b>.</p>
                        <p><b>⚙️ Modo:</b> Confirmada asociación espacial-temporal basada en el volumen de tráfico solapado durante la franja de los hechos.</p>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    if not df_m.empty:
                        df_agrup_m = df_m.groupby(['latitud', 'longitud']).apply(lambda x: pd.Series({
                            'visitas': len(x),
                            'data_html': "".join([f"<b>Línea {num}</b>: {len(x[x['numero_limpio']==num])} impactos | Celda: {', '.join([str(c) for c in x[x['numero_limpio']==num]['celda'].unique() if c!='Desconocida'])} | Fechas: {x[x['numero_limpio']==num]['fecha_limpia'].min()} a {x[x['numero_limpio']==num]['fecha_limpia'].max()}<hr style='margin:2px;'>" for num in x['numero_limpio'].unique()])
                        })).reset_index()
                        ...
                        for rank_idx, (_, r) in enumerate(df_agrup_m.sort_values('visitas', ascending=False).iterrows()):
                            ...
                            folium.CircleMarker([r['latitud'], r['longitud']], radius=min(20, 5+(r['visitas']/2)), color='darkred', fill=True).add_to(m)
                        ...
                        for obj in df_m['numero_limpio'].unique():
                            ...
                            for _, r in df_kml_obj.iterrows():
                                ...
                                pnt.description = f"Fecha: {r['fecha_limpia']}<br>Antena: {r['nombre_antena']}"
                    ...
                    st.warning("Los números existen, pero no registran actividad en las Fechas y Horas seleccionadas.")
            else:
        ...
        if st.session_state.run_t4:
            if len(vic_num) >= 7:
                with st.spinner("Correlacionando mallas de ubicación..."):
                    ...
                    # Redondeo Geográfico (Aprox 100m de radio)
                    ...
                    if not df_v.empty:
                        ...
                        if res:
                            ...
                            st.markdown(f'<div class="caja-alerta">💡 <b>Sospechosos Identificados:</b> El algoritmo señala a <b>{sosp.iloc[0]["numero_limpio"]}</b> como principal victimario, registrando encuentros y coincidencias certeras en <b>{sosp.iloc[0]["ubicaciones_distintas"]}</b> ubicaciones distintas dentro del margen de {tol}.</div>', unsafe_allow_html=True)
                        ...
                        st.warning("La víctima no registra actividad bajo las fechas y horas filtradas.")
    ...
    with tab5:
        ...
        d_i_5 = cf1.date_input("Fecha Inicio", f_min, min_value=MIN_DATE, max_value=MAX_DATE, key="t5_d1")
        t_i_5 = cf2.time_input("Hora Inicio", datetime.time(0, 0), key="t5_t1")
        d_f_5 = cf3.date_input("Fecha Fin", f_max, min_value=MIN_DATE, max_value=MAX_DATE, key="t5_d2")
        t_f_5 = cf4.time_input("Hora Fin", datetime.time(23, 59), key="t5_t2")
