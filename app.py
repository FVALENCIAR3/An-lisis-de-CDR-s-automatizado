...
# Limites globales de calendario (Liberados para buscar en el pasado/futuro sin error)
...
if st.session_state.df_master is not None:
            ...
            if lista_nums:
                ...
                if not df_u.empty:
                    ...
                    for rank_idx, (_, row) in enumerate(df_coords.iterrows()):
                        rank = rank_idx + 1
                        # FUCSIA si convergen múltiples objetivos en el mismo punto
        ...
        if st.session_state.run_t2:
            with st.spinner("Procesando redes y topologías..."):
                ...
                if not df_ana.empty:
                    ...
                    with colB:
                        st.subheader(f"📍 Encuentros ({tol_enc} - Por Operador)")
    ...
    with tab3:
        ...
        st.markdown("Extrae entidades (líneas) de documentos anexos y correlaciona Modo, Tiempo y Lugar exactos basados en la tabla.")
            ...
            if nums_implicados:
                ...
                if not df_c.empty:
                    ...
                    <div class="caja-info">
                        ...
                        <p><b>⚙️ Modo:</b> Confirmada asociación espacial-temporal basada en el volumen de tráfico solapado durante la franja estricta de los hechos ({dt_i} al {dt_f}).</p>
                    </div>
                    """, unsafe_allow_html=True)
                    
                    st.subheader("Tablas Explicativas (Actividad Detectada)")
                    ...
                    if not df_m.empty:
                        ...
                        st.download_button("🌍 Descargar Ruta Georeferenciada (KMZ/KML)", data=kml.kml(), file_name="Rutas_Correlacionadas.kml", mime="application/vnd.google-earth.kml+xml")
                ...
                with st.spinner("Correlacionando mallas de ubicación..."):
                    ...
                    if not df_v.empty:
                        ...
                        if res:
                            ...
                            st.markdown(f'<div class="caja-alerta">💡 <b>Sospechosos Identificados:</b> Tras el análisis de la franja {dt_i} al {dt_f}, el algoritmo señala a <b>{sosp.iloc[0]["numero_limpio"]}</b> como principal victimario, interceptando a la víctima en <b>{sosp.iloc[0]["ubicaciones_distintas"]}</b> ubicaciones/celdas distintas en el rango de {tol}.</div>', unsafe_allow_html=True)
                            st.dataframe(sosp, use_container_width=True, hide_index=True)
                        else:
                            st.success("Cruce Negativo: Ninguna línea siguió a la víctima en ese rango espacial-temporal de fechas.")
                    else:
    ...
    with tab5:
        ...
        st.markdown("Extrae la **ficha técnica completa** para informes periciales, y analiza el patrón de vida individual y cruzado en el rango de fechas exigido.")
        ...
        if st.session_state.run_t5:
            ...
            if list_obj:
                ...
                if not df_obj.empty:
                    ...
                    txt_sinop = "<b>Análisis de Rutinas (Aplicando Filtros Exigidos):</b><br>"
                    for obj in list_obj:
    ...
    with tab6:
        ...
        if btn_kml_3d:
            ...
            colors = {
                "Central (Max Power)": simplekml.Color.hexa('990000ff'), # Rojo con opacidad
                "Upper (-3dB)": simplekml.Color.hexa('99ff0000'),        # Azul con opacidad
                "Lower (-3dB)": simplekml.Color.hexa('9900ff00')         # Verde con opacidad
            }
            ...
            for p_name, a_deg in angles.items():
                ...
                pol.extrude = 0
                pol.style.polystyle.color = colors[p_name]
