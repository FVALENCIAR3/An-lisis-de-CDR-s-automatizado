...
if st.session_state.df_master is not None:
            ...
            if lista_nums:
                ...
                if not df_u.empty:
                    ...
                    for rank_idx, (_, row) in enumerate(df_coords.iterrows()):
                        rank = rank_idx + 1
                        # FUCSIA si hay más de 1 objetivo
        ...
        if st.session_state.run_t2:
            with st.spinner("Procesando redes y topologías..."):
                ...
                if not df_ana.empty:
                    ...
                    # Corrección de Frecuencia Pandas >= 2.2.0 (min, h)
                    tol_p = tol_enc.lower() 
    ...
    with tab3:
        ...
        st.markdown("Extrae entidades (líneas) de documentos anexos y correlaciona Modo, Tiempo y Lugar exactos basados en la tabla. Incluye descarga KML.")
                ...
                with st.spinner("Correlacionando mallas de ubicación..."):
                    ...
                    if not df_v.empty:
                        ...
                        if res:
                            ...
                            st.markdown(f'<div class="caja-alerta">💡 <b>Sospechosos Identificados:</b> El algoritmo señala a <b>{sosp.iloc[0]["numero_limpio"]}</b> como principal victimario, interceptando a la víctima en <b>{sosp.iloc[0]["ubicaciones_distintas"]}</b> ubicaciones distintas dentro del margen de {tol}.</div>', unsafe_allow_html=True)
