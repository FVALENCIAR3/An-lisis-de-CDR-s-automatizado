    ...
    if st.button("🗺️ Trazar mapa", type="primary", key="geo_btn"):
        if not re.fullmatch(r"\d{10}", numero.strip()):
            ...
            if d.empty:
                ...
                # ============================================================
                # AGRUPACIÓN ROBUSTA DE COORDENADAS
                # ============================================================

                columnas_obligatorias = {"latitud", "longitud", "fecha_limpia"}
                faltantes = columnas_obligatorias - set(d.columns)

                if faltantes:
                    st.error(
                        "❌ No se pueden generar las ubicaciones porque "
                ...
                d["fecha_limpia"] = pd.to_datetime(
                    d["fecha_limpia"], errors="coerce"
                )
                d = d.dropna(
                    subset=["latitud", "longitud", "fecha_limpia"]
                )
                ...
                if "celda" in d.columns:
                    ...
                    agrupado["celdas"] = "Sin información"
