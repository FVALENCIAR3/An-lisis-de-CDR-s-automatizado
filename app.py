...
def procesar_archivos(uploaded_files):
    ...
    # Buscador de columnas robusto (Acepta coincidencias exactas o partes de la palabra)
    ...
    for file in uploaded_files:
        try:
            ...
            # Diccionarios de Columnas Técnicas Actualizados
            ...
            # Datos de Antena (Haz, Azimut, Lac)
...
if submit_button:
    if uploaded_files:
        with st.spinner("Limpiando, estandarizando y consolidando data (Hex, Fechas, Coordenadas, Azimut, Haz)..."):
...
if st.session_state.df_master is not None:
        ...
        if st.session_state.run_t1:
            ...
            if lista_nums:
                ...
                if not df_u.empty:
                    ...
                    # Agrupar por Coordenadas para solucionar Overlapping
                    df_coords = df_u.groupby(['latitud', 'longitud']).apply(lambda x: pd.Series({
                        'total_visitas': len(x),
                        'objetivos_presentes': list(x['numero_limpio'].unique()),
                        'data_html': "".join([
                            f"<b>Línea:</b> <span style='color:{mapa_colores.get(num, 'black')}'>{num}</span> | <b>Impactos:</b> {len(x[x['numero_limpio']==num])}<br>"
                            f"<b>Celda(s):</b> {', '.join([c for c in x[x['numero_limpio']==num]['celda'].astype(str).unique() if c not in ('nan', 'Desconocida')])}<br>"
                    ...
                    st.markdown('<div class="caja-info">💡 <b>Sinopsis Geográfica:</b> Mapa interactivo generado. Si varios objetivos coincidieron en la misma antena/coordenada, el punto aparecerá oscuro. Haz clic sobre los puntos para desplegar la ficha de tiempos y visitas de CADA objetivo en ese lugar.</div>', unsafe_allow_html=True)
                    ...
                    for rank_idx, (_, row) in enumerate(df_coords.iterrows()):
                        rank = rank_idx + 1
                        # Si hay más de un objetivo en el mismo punto, colorear de negro para indicar "Choque"
                        color = 'darkred' if len(row['objetivos_presentes']) > 1 else mapa_colores.get(row['objetivos_presentes'][0], 'gray')
        ...
        if st.session_state.run_t2:
            with st.spinner("Procesando redes y topologías..."):
                ...
                if not df_ana.empty:
                    ...
                    if not top_e.empty: s_txt += f"Se constata un probable encuentro recurrente entre <b>{top_e.iloc[0]['Número A']}</b> y <b>{top_e.iloc[0]['Número B']}</b> ({top_e.iloc[0]['Match']} cruces confirmados en 15 min)."
            ...
            if nums_implicados:
                ...
                if not df_c.empty:
                    ...
                    <div class="caja-info">
                        ...
                        <p><b>Números Correlacionados:</b> {', '.join(df_c['numero_limpio'].unique())} ({n_activos} activos encontrados en el relato).</p>
                        <p><b>⏱️ Tiempo:</b> Operatividad confirmada entre el <b>{df_c['fecha_limpia'].min()}</b> y el <b>{df_c['fecha_limpia'].max()}</b>, con <b>{len(df_c)}</b> trazas en red.</p>
                        <p><b>📍 Lugar:</b> Convergencia principal en celdas: <b>{', '.join([str(c) for c in celdas_comunes if c != 'Desconocida'])}</b>.</p>
                        <p><b>⚙️ Modo:</b> Confirmada asociación espacial-temporal basada en el volumen de tráfico solapado.</p>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.warning("Los números existen, pero no registran actividad en las Fechas seleccionadas.")
            else:
                st.error("No se detectaron móviles válidos (10 dígitos, inicio 3) en el relato.")
        ...
        if st.session_state.run_t4:
            if len(vic_num) >= 7:
                with st.spinner("Correlacionando mallas de ubicación..."):
                    ...
                    # Redondeo Geográfico (Aprox 100m de radio) para atrapar sospechosos en antenas compartidas
                    ...
                    if not df_v.empty:
                        ...
                        if res:
                            ...
                            st.markdown(f'<div class="caja-alerta">💡 <b>Sospechosos Identificados:</b> Señalamos a <b>{sosp.iloc[0]["numero_limpio"]}</b> como principal victimario. Interceptó a la víctima en <b>{sosp.iloc[0]["ubicaciones_distintas"]}</b> ubicaciones distintas dentro del margen de {tol}.</div>', unsafe_allow_html=True)
                            st.dataframe(sosp, use_container_width=True, hide_index=True)
                        else:
                            st.success("Cruce Negativo: Ninguna línea siguió a la víctima.")
                    else:
                        st.warning("La víctima no registra actividad bajo estas fechas.")
    ...
    with tab5:
        ...
        st.markdown("Extrae la **ficha técnica completa** para informes periciales, y analiza el patrón de vida individual y cruzado.")
