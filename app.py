...
def procesar_archivos(uploaded_files):
    ...
    for file in uploaded_files:
        try:
            ...
            # 1. Leer CSV o TXT (Con reinicio de puntero de lectura file.seek)
            if file_ext.endswith(('.csv', '.txt')):
                file.seek(0)
                try: 
                    # Intento 1: Punto y coma
                    df = pd.read_csv(file, sep=';', encoding='utf-8-sig', low_memory=False, on_bad_lines='skip')
                ...
                if len(df.columns) <= 1:
                    # Intento 3: Tabulación (Muy común en TXT de telefónicas)
                    file.seek(0)
                    df = pd.read_csv(file, sep='\t', encoding='utf-8-sig', low_memory=False, on_bad_lines='skip')
...
# ==========================================
# BARRA LATERAL (CARGA DE DATOS SEGURA)
# ==========================================
st.sidebar.title("📡 SIGINT App")
st.sidebar.markdown("---")

# Formularios: Evita que Streamlit colapse al subir muchos archivos
...
if submit_button:
    if uploaded_files:
        with st.spinner("Procesando gigabytes de datos..."):
        ...
        if st.session_state.df_master is not None and not st.session_state.df_master.empty:
            st.sidebar.success(f"✅ {len(st.session_state.df_master)} registros cargados y estandarizados con éxito.")
        else:
            st.sidebar.error("❌ Los archivos se leyeron, pero no contienen cabeceras telefónicas compatibles.")
    else:
        st.sidebar.warning("⚠️ Sube al menos un archivo antes de procesar.")
...
if st.session_state.df_master is not None:
    ...
    st.info("👈 Por favor, carga las bases de datos en el menú lateral y dale a 'Procesar Archivos'.")