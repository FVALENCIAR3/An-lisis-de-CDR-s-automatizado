...
def load_and_process_files(uploaded_files_data):
    ...
    for file_name, file_content in uploaded_files_data:
        try:
            ...
            # Crear SIEMPRE las columnas normalizadas para evitar
            # KeyError cuando una base no trae coordenadas/celda/teléfono.
            ...
            # Preservar campos descriptivos cuando existan.
            for alias in ["descripcion", "direccion", "municipio", "departamento"]:
                if alias in df.columns:
                    df_temp[alias] = df[alias].astype(str)
    ...
    # Tipos consistentes para evitar errores entre archivos heterogéneos.
    if "numero_limpio" in df_master.columns:
        df_master["numero_limpio"] = df_master["numero_limpio"].astype("string")
    if "celda" in df_master.columns:
        df_master["celda"] = df_master["celda"].astype("string")
    if "fecha_limpia" in df_master.columns:
...
def procesar_analisis(
    ...
    st.dataframe(top_10_celdas, width="stretch")

    st.subheader("📍 Top 10: Encuentros Probables (Espacio/Tiempo)")
    st.dataframe(df_encuentros, width="stretch")
...
def rastrear_victimarios(
    ...
    st.dataframe(sospechosos, width="stretch")
...
def analizar_rutinas(
    ...
    if "descripcion" in df_obj.columns:
        df_obj["_nombre_lugar"] = df_obj["descripcion"].fillna("").astype(str)
    elif "direccion" in df_obj.columns:
        df_obj["_nombre_lugar"] = df_obj["direccion"].fillna("").astype(str)
    else:
        df_obj["_nombre_lugar"] = df_obj["celda"].fillna("").astype(str)

    rutinas = (
        df_obj.groupby(["franja", "celda"], dropna=False)
        .agg(
            nombre_lugar=("_nombre_lugar", "first"),
            visitas=("celda", "count"),
    ...
    if simplekml is not None:
        if st.button("🌎 Generar KML para Google Earth", key="btn_kml"):
            ...
            # simplekml.Kml.save() exige una ruta de archivo.
            # Para Streamlit es más estable usar el XML generado por kml.kml().
            kml_data = kml.kml()
