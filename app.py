...
def procesar_archivos(uploaded_files):
    ...
    # Consolidación y Blindaje de Columnas
    if df_list: 
        df_final = pd.concat(df_list, ignore_index=True)
        # BLINDAJE ESTRUCTURAL: Garantizar que TODAS las columnas existan, incluso si el archivo no las traía
        columnas_base = ['numero_limpio', 'fecha_limpia', 'latitud', 'longitud', 'celda', 'lac', 'haz', 'azimut', 'nombre_antena', 'operador', 'registro_original']
        for col in columnas_base:
            if col not in df_final.columns:
                if col in ['latitud', 'longitud']:
                    df_final[col] = np.nan
                elif col == 'fecha_limpia':
                    df_final[col] = pd.NaT
                elif col == 'celda':
                    df_final[col] = 'Desconocida'
                elif col == 'registro_original':
                    df_final[col] = {}
                else:
                    df_final[col] = 'N/A'
        return df_final
            ...
            if nums_implicados:
                ...
                if not df_c.empty:
                    ...
                    <div class="caja-info">
                        ...
                        <p><b>Números Correlacionados:</b> {', '.join(color_labels)} ({n_activos} activos encontrados en el relato operando en este rango).</p>
                ...
                if not df_obj.empty:
                    ...
                    for obj in list_obj:
                        ...
                        if not df_o_r.empty:
                            ...
                            dia = df_o_r[df_o_r['franja'].str.contains('DÍA|MAÑANA|TARDE')]
    ...
    with tab6:
        ...
        if btn_kml_3d:
            ...
            for p_name, a_deg in angles.items():
                ...
                # COLOR: Amarillo al 40% transparente (KML AABBGGRR -> 66 = 40% Alpha, 00=B, FF=G, FF=R)
